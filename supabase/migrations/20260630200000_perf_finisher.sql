-- Finisher: applies the still-pending perf migration (20260630120000) and
-- removes the 4 duplicate unique constraints that hardening v2 accidentally
-- added (those tables already had identical `<table>_<cols>_key` constraints
-- from the original schema — the originals are kept).
--
...
-- ─────────────────────────────────────────────────────────────────────────
-- 0. Drop the duplicate unique constraints from hardening v2
--    (keeps the pre-existing originals: *_key)
-- ─────────────────────────────────────────────────────────────────────────
alter table public.follows         drop constraint if exists follows_unique;
alter table public.round_likes     drop constraint if exists round_likes_unique;
alter table public.favourites      drop constraint if exists favourites_unique;
alter table public.society_members drop constraint if exists society_members_unique;

-- ─────────────────────────────────────────────────────────────────────────
-- 1. RLS: wrap auth.uid() -> (select auth.uid())  [advisor: auth_rls_initplan]
--    Zero behaviour change; evaluated once per query instead of per row.
-- ─────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────
-- 2. Covering indexes for foreign keys  [advisor: unindexed_foreign_keys]
-- ─────────────────────────────────────────────────────────────────────────
